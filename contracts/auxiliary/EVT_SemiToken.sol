...
contract EVT_SemiToken is IERC20 {
    ...
    string public name = "EVT_SemiToken";
    string public symbol = "EVTST";
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        return IMultiHonor(honor).EventPoint(tokenId);
    }
    ...
    function transfer(address to, uint256 amount) external returns (bool) {
        ...
        ids[0] = tokenId;
        uint64[] memory evts = new uint64[](1);
        evts[0] = uint64(amount);
        IMultiHonor(honor).addEventPoint(ids, evts);
        _totalSupply += amount;
